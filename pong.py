...
class Player:
    ...
    def select_action(self, state):
        ...
        if rate > random.random():
            return torch.tensor(random.randrange(self.num_actions)).to(
                self.device
            )  # explore
        else:
            with torch.no_grad():
...
class Ball:
    ...
    def collision(self, player1, player2):
        ...
        reward2 = 0
        if (
            self.x + self.r >= player1.x
            and self.x - self.r <= player1.x + player1.width
        ):
            if (
                self.y + self.r >= player1.y
                and self.y + self.r <= player1.y + player1.height
            ):
                collision = True
                P1 = True

        if (
            self.x - self.r <= player2.x + player2.width
            and self.x + self.r >= player2.x
        ):
            if (
                self.y + self.r >= player2.y
                and self.y + self.r <= player2.y + player2.height
            ):
                collision = True
                P2 = True
        ...
        if collision & P1:
            ...
            if self.y + self.r > player1.y and self.y <= math.floor(
                player1.y + player1.height / 8
            ):
                x = 1
            if self.y > math.floor(
                player1.y + player1.height / 8
            ) and self.y <= math.floor(player1.y + 3 * player1.height / 8):
                x = 2
            if self.y > math.floor(
                player1.y + 3 * player1.height / 8
            ) and self.y <= math.floor(player1.y + 5 * player1.height / 8):
                x = 3
            if self.y > math.floor(
                player1.y + 5 * player1.height / 8
            ) and self.y <= math.floor(player1.y + 7 * player1.height / 8):
                x = 4
            if self.y > math.floor(
                player1.y + 7 * player1.height / 8
            ) and self.y - self.r <= math.floor(player1.y + player1.height):
                x = 5
            reward1 = 0
        if collision & P2:
            self.x = player2.x + player2.width + self.r
            if self.y + self.r > player2.y and self.y <= math.floor(
                player2.y + player2.height / 8
            ):
                x = 1
            if self.y > math.floor(
                player2.y + player2.height / 8
            ) and self.y <= math.floor(player2.y + 3 * player2.height / 8):
                x = 2
            if self.y > math.floor(
                player2.y + 3 * player2.height / 8
            ) and self.y <= math.floor(player2.y + 5 * player2.height / 8):
                x = 3
            if self.y > math.floor(
                player2.y + 5 * player2.height / 8
            ) and self.y <= math.floor(player2.y + 7 * player2.height / 8):
                x = 4
            if self.y > math.floor(
                player2.y + 7 * player2.height / 8
            ) and self.y - self.r <= math.floor(player2.y + player2.height):
                x = 5
            reward2 = 0
...
class Pong_env:
    ...
    def __init__(self):
        ...
        self.ball = Ball(
            math.floor(self.screen_width / 2),
            math.floor(self.screen_width / 2),
            6,
            self.ball_speed_x,
            self.ball_speed_y,
    ...
    def step(self, actions):
        ...
        done, reward1, reward2 = self.score(
            self.player1_x, self.player1_y, self.player2_x, self.player2_y
        )
    ...
    def invert(state):
        ...
        return np.array(
            [
                p2_y,
                p1_y,
                getattr(Pong_env, "screen_width") / 800 - bx,
                by,
                -bvx,
                bvy,
                timestep,
...
class DQN(nn.Module):
    def __init__(self, input_size=6, output_size=3, device=torch.device("cpu")):
